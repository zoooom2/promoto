...
const PricingCards = ({
	...
	return (
		<div
			className={`w-full relative flex justify-center items-center poverflow-clip content-center gap-2.5 border border-[rgba(0,27,16,0.1)] rounded-[20px] ${
				withBg && 'bg-[#001b10]'
			}`}
		>
			<div className='flex-1 flex flex-col py-11 px-9 overflow-clip flex-nowrap gap-2.5'>
				<div className='w-full flex flex-col items-center content-center flex-nowrap gap-12'>
								...
								<div className='w-full flex justify-start items-end content-end flex-nowrap'>
									<div className='flex justify-start items-center content-center flex-nowrap'>
										{custom ? (
											<h5
												className={`whitespace-pre ${
													withBg ? 'text-[#f2ff00]' : 'text-[#066f41]'
					...
					<Button
						className={`w-full py-2 rounded-[8px] capitalize flex justify-center items-center overflow-hidden content-center flex-nowrap gap-2.5 font-hanken ${
							withBg ? 'text-[#001b10] bg-[#f2ff00]' : 'text-white bg-[#001b10]'
						}`}
					>
						{button}
					</Button>
				...
				{tag && (
					<Button className='absolute top-2 right-2 flex justify-center py-1 px-3 bg-[#f2ff00] z-1 content-center flex-nowrap gap-2 rounded-[12px] text-[#001b10] capitalize whitespace-pre font-hanken text-[16px] trcking-[-0.04em] text-center leading-[1.6]'>
						most popular
					</Button>
				)}
			</div>
